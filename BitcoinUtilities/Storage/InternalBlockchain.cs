...
namespace BitcoinUtilities.Storage
{
    ...
    public class InternalBlockchain
    {
        ...
        public bool Include(byte[] hash)
        {
            ...
            UnspentOutputsUpdate unspentOutputsUpdate = new UnspentOutputsUpdate(storage);
            try
            {
                TransactionProcessor txProcessor = new TransactionProcessor();
                txProcessor.UpdateOutputs(unspentOutputsUpdate, block.Height, block.Hash, blockMessage);
            }
